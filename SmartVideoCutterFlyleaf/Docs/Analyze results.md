...
| 3  | `Keyframe` не реализует `INotifyPropertyChanged` → чекбокс `IsSelected` в DataGrid не обновляется                                                                                           | `MainViewModel.cs:26`, `Models/Keyframe.cs` |
...
9. **Нейминг/структура**: `Initialize(Dispatcher dispatcher)` принимает диспетчер, но не использует его.
10. **Документация устарела**: `Docs/Architecture.md` описывает **LibVLC** («LibVLC TakeSnapshot ()») — это копия из
...
3. **ONNX**: добавить `GraphOptimizationLevel.ORT_ENABLE_ALL`, задать `IntraOp/InterOpNumThreads`; для DML — `DeviceId`.
4. **Буферы**: портировать `TensorConverter` с переиспользуемыми массивами (уже есть в WinForms-проекте).
5. **`LogLevel.Debug`** в `EngineConfig` в release — шумно; сделать условным.
...
3. **Статус-бар** (ошибки, прогресс анализа/экспорта, FPS/разрешение из `Player.Video`).
4. **ProgressDialog без кнопки «Отмена»** — добавить `CancellationToken`.
5. **Локализация**: интерфейс смешанный (меню по-русски, кнопки «Play/Pause», «Analyze», «Export», заголовки колонок
   по-английски) — унифицировать.
6. **Опционально**: заменить самописный плеер-UI на готовый контрол **`FlyleafME`** (FlyleafLib.Controls.WPF уже
...
7. Заголовок окна с именем файла; «О программе» без команды.
...
4. Подписка на `Player.PropertyChanged` (`CurTime`, `Duration`, `Status`, `LastError`) вместо таймера; убрать
